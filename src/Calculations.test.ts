...
test('operation about string and calculation "1 + 2 = 3"', () => {
    expect(Calculations.multipleArithmeticOperation("1 + 2")).toBe(3)
});

test('multiple arythmetic operations: "1+2*3/2"', () => {
    expect(Calculations.multipleArithmeticOperation("1 + 2 * 3 / 2")).toBe(4);
})
test('multiple arythmetic operations: "2*2*3/2"', () => {
    expect(Calculations.multipleArithmeticOperation("2 * 2 * 3 / 2")).toBe(6);
})
test('multiple arythmetic operations: "1+2)3/2" with error: Undefined operator', () => {
    expect(() => Calculations.multipleArithmeticOperation("1 + 2 ) 3 / 2")).toThrow('Undefined operator');
})
test('multiple arythmetic operations: "1+2)3/2" with error: Number cannot be 0, please provide another', () => {
    expect(() => Calculations.multipleArithmeticOperation("0 + 0 / 3 / 2")).toThrow("Number can't be 0, please provide another");
})
