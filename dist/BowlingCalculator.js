export class BowlingCalculator {
    ...
    static getBowlingResults(bowlingResults) {
        return bowlingResults.map(result => result);
    }
    static bowlingResultsCalculation(bowlingScoresList) {
        const bowlingResultList = BowlingCalculator.getBowlingResults(bowlingScoresList);
        const bowlingCalculationsList = [];
        for (let i = 0; i < bowlingResultList.length; i++) {
            if (bowlingResultList[i] === "X" && i < (bowlingResultList.length - 3)) {
                const bonus1 = BowlingCalculator.returningValueBowling(bowlingResultList[i + 1]);
                const bonus2 = BowlingCalculator.returningValueBowling(bowlingResultList[i + 2]);
    ...
    static returningValueBowling(char, previousValue) {
        if (char === 'X') {
            return 10;
        }
        else if (char === '-') {
